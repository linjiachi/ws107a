...
describe('多人網誌系統', function () {
  ...
  describe('GET /gigi/posts', function () { // 路徑 GET /
    it('內文標題應該為 <h1>gigi 的留言板</h1>，而且有 6 則貼文', function (done) {
      request.get('/gigi/posts').expect(200, function (err, res) {
        if (err) return done(err)

        expect(res.header['content-type']).to.include('html') // 根目錄是個 html 文件
        expect(res.text).to.include('<h1>Gig 的留言板</h1>')
        expect(res.text).to.include('<p>您總共有 <strong>6</strong> 則貼文!</p>')
        done()
      })
    })
  })

  describe('GET /gigi/post/0', function () { // 路徑 GET /
    it('內文標題應該為 <h2>Monday: go to school</h2>', function (done) {
      request.get('/gigi/post/0').expect(200, function (err, res) {
        if (err) return done(err)

        expect(res.header['content-type']).to.include('html') // 根目錄是個 html 文件
        expect(res.text).to.include('<h2>Monday: go to school</h2>')
        done()
      })
    })
  })
